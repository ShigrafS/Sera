...
using TaskSage.Data;
using TaskSage.Core.Models;
using Windows.Security.Credentials;

namespace TaskSage.Services;
...
public class NvidiaInferenceService : INvidiaInferenceService
{
    ...
    private readonly TaskSageDbContext _db;

    public NvidiaInferenceService(HttpClient httpClient, TaskSageDbContext db)
    {
    ...
    private string? GetApiKey()
    {
        try
        {
            ...
            // We use "TaskSageNvidia" as the resource name, "ApiKey" as the user name
            var credential = vault.Retrieve("TaskSageNvidia", "ApiKey");
    ...
    {
        var settings = await _db.Settings.FirstOrDefaultAsync() ?? new TaskSage.Data.Entities.AppSettings();
